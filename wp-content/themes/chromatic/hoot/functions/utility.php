...
add_action( 'init', 'chromaticfw_add_post_type_support' );
 ...
 * @return void
 */
function chromaticfw_add_post_type_support() {
...
add_filter( 'the_title', 'chromaticfw_untitled_post' );
 ...
 * @return string
 */
function chromaticfw_untitled_post( $title ) {
 ...
 * @return string
 */
function chromaticfw_locate_theme_file( $file_names ) {
 ...
 * @return array
 */
function chromaticfw_hex_to_rgb( $hex ) {
 ...
 * @return array
 */
function chromaticfw_trim( $content ) {
 ...
 * @return array
 */
function chromaticfw_array_insert( $insert, $target, $location, $order = 'before' ) {
 ...
 * @return string
 */
function chromaticfw_get_menu_location_name( $location ) {
 ...
 * @return string
 */
function chromaticfw_get_sidebar_name( $sidebar_id ) {
 ...
 * @return string
 */
function chromaticfw_get_min_suffix() {
	return defined( 'CHROMATICFW_DEBUG' ) && CHROMATICFW_DEBUG ? '' : '.min';
}
 ...
 * altered to display a custom page as 404 page by the ChromaticFw framework.
 *
 * @since 1.0.0
 * @access public
 * @return bool
 */
function chromaticfw_is_404() {
	global $chromaticfw;
	if ( isset( $chromaticfw->is_404 ) && is_bool( $chromaticfw->is_404 ) )
		return $chromaticfw->is_404;
	else
		return is_404();
}
 ...
 * Ideally, CHROMATICFW_PREMIUM should be set to (bool)true at the start of '/hoot-theme/hoot-theme.php' file.
 *
 * @since 1.1.0
 * @access public
 * @return bool
 */
function chromaticfw_is_premium() {
	$return = ( defined( 'CHROMATICFW_PREMIUM' ) && CHROMATICFW_PREMIUM === true );
	return $return;
}