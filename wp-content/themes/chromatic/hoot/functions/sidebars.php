		...
		for ( $i=0; $i < $columns; $i++ ) :
			if ( isset( $alphas[ $i ] ) ) :
				chromaticfw_register_sidebar(
					array(
						'id'          => 'footer-' . $alphas[ $i ],
						'name'        => sprintf( _x( 'Footer %s', 'sidebar', 'chromatic' ), strtoupper( $alphas[ $i ] ) ),
						'description' => __( 'You can set footer columns in Theme Options page.', 'chromatic' )
					)
				);
			endif;
		endfor;
...
function chromaticfw_widgetized_template_register_sidebars() {
	...
	if ( current_theme_supports( 'chromaticfw-widgetized-template' ) ) :
		...
		$defaults = array(
			'area_a' => __('Widgetized Template - Area A', 'chromatic'),
			'area_b' => __('Widgetized Template - Area B', 'chromatic'),
			'area_c' => __('Widgetized Template - Area C', 'chromatic'),
			'area_d_1' => __('Widgetized Template - Area D Left', 'chromatic'),
			'area_d_2' => __('Widgetized Template - Area D Right', 'chromatic'),
		);
		...
		foreach ( $defaults as $key => $name ) {
			chromaticfw_register_sidebar(
				array(
					...
					'name'        => $name,
					'description' => __( 'You can order Widgetized Template areas in Theme Options page.', 'chromatic' )
				)
			);
		}
