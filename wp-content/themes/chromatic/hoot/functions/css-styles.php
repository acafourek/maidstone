 ...
 * @return void|string
 */
function chromaticfw_css_rule( $style, $value, $echo = false, $important = false ) {
	...
	if ( !function_exists( 'chromaticfw_of_sanitize_enum' ) )
	...
	switch( $style ):
		...
		case 'border-left-color':
			if ( 'none' == $value || 'transparent' == $value )
				...
				// sanitize color. chromaticfw_of_sanitize_hex() will return null if $value is not a formatted hex color
				$value = chromaticfw_of_sanitize_hex( $value );
			break;

		case 'background':
			if ( is_array( $value ) ) {
				// use the chromaticfw_css_background function for multiple background properties
				chromaticfw_css_background( $value );
				return;
			} elseif ( 'none' == $value || 'transparent' == $value ) {
				$value = 'none';
			} else {
				// sanitize for background color. chromaticfw_of_sanitize_hex() will return null if $value is not a formatted hex color
				$value = chromaticfw_of_sanitize_hex( $value );
			}
			break;
		...
		case 'font':
			if ( is_array( $value ) ) {
				// use the chromaticfw_css_typography function for multiple font properties
				chromaticfw_css_typography( $value );
				return;
			} else {
				// Recognized font-families in hoot/options/includes/fonts{-google}.php
				$recognized = chromaticfw_of_recognized_font_faces();
...
function chromaticfw_css_background( $background ) {
	...
	if ( 'predefined' == $type ) {
		if ( !empty( $pattern ) ) { // skip if pattern = 0 (i.e. user has selected 'No Pattern')
			...
			if ( array_key_exists( $pattern, $recognized ) ) {
				$background_image = trailingslashit( CHROMATICFW_IMAGES ) . 'patterns/' . $pattern;
	...
	if ( 'custom' == $type ) {
		if ( !empty( $image ) ) {
			$output .= chromaticfw_css_rule( 'background-image', $image );
			if ( !empty( $repeat ) ) {
				$output .= chromaticfw_css_rule( 'background-repeat', $repeat );
			}
			if ( !empty( $position ) ) {
