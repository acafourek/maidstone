...
add_filter( 'image_size_names_choose', 'chromaticfw_image_size_names_choose' );

/* Adds ID3 tags for media display. */
add_filter( 'wp_get_attachment_id3_keys', 'chromaticfw_attachment_id3_keys', 5, 3 );

/* Register custom image sizes. */
add_action( 'init', 'chromaticfw_register_image_sizes', 5 );
 ...
 * @return array
 */
function chromaticfw_image_size_names_choose( $wp_sizes ) {
	$sizes = array();
	$sizes = apply_filters( 'chromaticfw_custom_image_sizes', $sizes );
 ...
 * @return void
 */
function chromaticfw_register_image_sizes() {
	$sizes = array();
	$sizes = apply_filters( 'chromaticfw_custom_image_sizes', $sizes );
 ...
 * @return string
 */
function chromaticfw_get_image_size_name( $span, $crop=NULL, $gridadjust=0 ) {
	$default_grid = 1260;
	...
	/* Get the Grid (int)Width from ChromaticFw Options else Default */
	$grid = ( function_exists( 'chromaticfw_get_option' ) ) ? intval( chromaticfw_get_option( 'site_width' ) ) : 0;
	if ( empty( $grid ) )
		$grid = $default_grid;
	$grid -= $gridadjust;
	...
	if ( $crop === true )
		$iwidths = chromaticfw_get_image_sizes( 'sort_by_width_crop' );
	elseif ( $crop === false )
		$iwidths = chromaticfw_get_image_sizes( 'sort_by_width_nocrop' );
	else
		$iwidths = chromaticfw_get_image_sizes( 'sort_by_width' );
 ...
 * @return array
 */
function chromaticfw_get_image_sizes( $return = '' ) {
 ...
 * @return array
 */
function chromaticfw_attachment_id3_keys( $fields, $attachment, $context ) {
	...
	if ( chromaticfw_attachment_is_audio( $attachment->ID ) ) {
