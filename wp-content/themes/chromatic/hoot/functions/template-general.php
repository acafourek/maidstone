...
function chromaticfw_get_wp_link() {
	return sprintf( '<a class="wp-link" href="http://wordpress.org" title="%s">%s</a>', esc_attr__( 'State-of-the-art semantic personal publishing platform', 'chromatic' ), __( 'WordPress', 'chromatic' ) );
}
 ...
 * @return void
 */
function chromaticfw_wp_theme_link( $link = '', $anchor = '' ) {
	echo chromaticfw_get_wp_theme_link( $link, $anchor );
}
 ...
 * @return string
 */
function chromaticfw_get_wp_theme_link( $link = '', $anchor = '' ) {
	...
	$anchor = ( empty( $anchor ) ) ? sprintf( __( '%s WordPress Theme', 'chromatic' ), $name ) : $anchor;

	$title = sprintf( __( '%s WordPress Theme', 'chromatic' ), $name );
 ...
 * @return void
 */
function chromaticfw_theme_link( $link = '', $anchor = '' ) {
	echo chromaticfw_get_theme_link( $link, $anchor );
}
 ...
 * @return string
 */
function chromaticfw_get_theme_link( $link = '', $anchor = '' ) {
...
function chromaticfw_get_loop_title() {
	...
	if ( ( ( $page = get_query_var( 'paged' ) ) || ( $page = get_query_var( 'page' ) ) ) && $page > 1 ) {
		...
		$loop_title_suffix = ' ' . sprintf( __( '(Page %s)', 'chromatic' ), $paged );
...
function chromaticfw_get_loop_description() {
	...
	elseif ( is_search() )
		$loop_desc = sprintf( __( 'You are browsing the search results for &#8220;%s&#8221;', 'chromatic' ), get_search_query() );
	...
	elseif ( is_day() )
		$loop_desc = sprintf( __( 'You are browsing the site archives for %s.', 'chromatic' ), chromaticfw_single_day_title( '', false ) );

	elseif ( is_month() )
		$loop_desc = sprintf( __( 'You are browsing the site archives for %s.', 'chromatic' ), single_month_title( ' ', false ) );

	elseif ( is_year() )
		$loop_desc = sprintf( __( 'You are browsing the site archives for %s.', 'chromatic' ), chromaticfw_single_year_title( '', false ) );
...
function chromaticfw_search_title( $prefix = '', $display = true ) {
	...
	$title = $prefix . sprintf( __( 'Search results for &#8220;%s&#8221;', 'chromatic' ), get_search_query() );
