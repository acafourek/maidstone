 ...
 * @return bool
 */
function chromaticfw_post_has_content( $id = 0 ) {
	$post = get_post( $id );
 ...
 * @return void
 */
function chromaticfw_post_format_link() {
	echo chromaticfw_get_post_format_link();
}
 ...
 * @return string
 */
function chromaticfw_get_post_format_link() {
 ...
 * @return void
 */
function chromaticfw_post_author( $args = array() ) {
	echo chromaticfw_get_post_author( $args );
}
 ...
 * @return string
 */
function chromaticfw_get_post_author( $args = array() ) {
	...
	if ( !empty( $link ) ) {
		$html .= $args['before'];
		$html .= sprintf( $args['wrap'], chromaticfw_get_attr( 'entry-author' ), sprintf( $args['text'], $link ) );
		$html .= $args['after'];
	}
 ...
 * @return void
 */
function chromaticfw_post_terms( $args = array() ) {
	echo chromaticfw_get_post_terms( $args );
}
 ...
 * @return string
 */
function chromaticfw_get_post_terms( $args = array() ) {
	...
	$defaults = array(
		...
		'sep'        => _x( ', ', 'taxonomy terms separator', 'chromatic' )
	);
	...
	if ( !empty( $terms ) ) {
		$html .= $args['before'];
		$html .= sprintf( $args['items_wrap'], chromaticfw_get_attr( 'entry-terms', $args['taxonomy'] ), sprintf( $args['text'], $terms ) );
		$html .= $args['after'];
	}
 ...
 * chromaticfw_get_gallery_image_count() function since it will count all gallery items regardless of mime type.
 *
 ...
 * @return int
 */
function chromaticfw_get_gallery_item_count() {
 ...
 * @return int
 */
function chromaticfw_get_gallery_image_count() {
 ...
 * @return string
 */
function chromaticfw_get_content_url( $content ) {
 ...
 * @return string
 */
function chromaticfw_get_the_post_format_url( $url = '', $post = null ) {

	if ( empty( $url ) ) {

		$post = is_null( $post ) ? get_post() : $post;

		$content_url = chromaticfw_get_content_url( $post->post_content );
