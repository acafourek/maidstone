 ...
 * @return bool
 */
function chromaticfw_attachment_is_audio( $post_id = 0 ) {
 ...
 * @return bool
 */
function chromaticfw_attachment_is_video( $post_id = 0 ) {
 ...
 * @credit Pippin Williamson http://pippinsplugins.com/retrieve-attachment-id-from-image-url/
 *
 * @since 1.0.0
 * @access public
 * @param string  $url
 * @return int
 */
function chromaticfw_get_attachment_id_from_url( $url ) {
	global $wpdb;
 ...
 * @return string
 */
function chromaticfw_get_image_size_links() {
 ...
 * @return string
 */
function chromaticfw_get_audio_transcript( $post_id = 0 ) {
	...
	return apply_filters( 'chromaticfw_audio_transcript', $lyrics );
}
 ...
 * @return void
 */
function chromaticfw_attachment() {
	...
	foreach ( $mime_type as $type ) {
		if ( function_exists( "chromaticfw_{$type}_attachment" ) )
			$attachment = call_user_func( "chromaticfw_{$type}_attachment", $mime, $file );

		$attachment = apply_filters( "chromaticfw_{$type}_attachment", $attachment );
	}

	echo apply_filters( 'chromaticfw_attachment', $attachment );
}
 ...
 * @return string
 */
function chromaticfw_application_attachment( $mime = '', $file = '' ) {
 ...
 * @return string
 */
function chromaticfw_text_attachment( $mime = '', $file = '' ) {
	$embed_defaults = wp_embed_defaults();
 ...
 * @return string
 */
function chromaticfw_audio_attachment() {
	return chromaticfw_media_grabber( array( 'type' => 'audio' ) );
}
 ...
 * @return string
 */
function chromaticfw_video_attachment() {
	return chromaticfw_media_grabber( array( 'type' => 'video' ) );
}
