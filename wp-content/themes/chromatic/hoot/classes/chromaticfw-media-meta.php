...
/**
 * Wrapper function for the ChromaticFw_Media_Meta class.
 *
 * @since 1.0.0
 * @access public
 * @param array $args
 * @return string
 */
function chromaticfw_media_meta( $args = array() ) {

	$meta = new ChromaticFw_Media_Meta( $args );
...
class ChromaticFw_Media_Meta {
	...
	public function __construct( $args = array() ) {
		...
		$this->args = apply_filters( 'chromaticfw_media_meta_args', wp_parse_args( $args, $defaults ) );
		...
		elseif ( chromaticfw_attachment_is_audio( $this->args['post_id'] ) )
			$this->audio_meta();

		/* If the attachment is video. */
		elseif ( chromaticfw_attachment_is_video( $this->args['post_id'] ) )
			$this->video_meta();
	}
	...
	public function image_meta() {
		...
		if ( !empty( $this->meta['width'] ) && !empty( $this->meta['height'] ) ) {
			...
			$this->items['dimensions'] = array(
				...
				'<a href="' . esc_url( wp_get_attachment_url() ) . '">' . sprintf( __( '%1$s &#215; %2$s', 'chromatic' ), number_format_i18n( absint( $this->meta['width'] ) ), number_format_i18n( absint( $this->meta['height'] ) ) ) . '</a>',
				__( 'Dimensions', 'chromatic' )
			);
		}

		/* If a timestamp exists, add it to the $items array. */
		if ( !empty( $this->meta['image_meta']['created_timestamp'] ) )
			$this->items['created_timestamp'] = array( date_i18n( get_option( 'date_format' ), strip_tags( $this->meta['image_meta']['created_timestamp'] ) ), __( 'Date', 'chromatic' ) );

		/* If a camera exists, add it to the $items array. */
		if ( !empty( $this->meta['image_meta']['camera'] ) )
			$this->items['camera'] = array( esc_html( $this->meta['image_meta']['camera'] ), __( 'Camera', 'chromatic' ) );

		/* If an aperture exists, add it to the $items array. */
		if ( !empty( $this->meta['image_meta']['aperture'] ) )
			$this->items['aperture'] = array( sprintf( '<sup>f</sup>&#8260;<sub>%s</sub>', absint( $this->meta['image_meta']['aperture'] ) ), __( 'Aperture', 'chromatic' ) );
		...
		if ( !empty( $this->meta['image_meta']['focal_length'] ) )
			...
			$this->items['focal_length'] = array( sprintf( __( '%s mm', 'chromatic' ), absint( $this->meta['image_meta']['focal_length'] ) ), __( 'Focal Length', 'chromatic' ) );
		...
		if ( !empty( $this->meta['image_meta']['iso'] ) ) {
			$this->items['iso'] = array(
				...
				'<abbr title="' . __( 'International Organization for Standardization', 'chromatic' ) . '">' . __( 'ISO', 'chromatic' ) . '</abbr>'
			);
		}
		...
		if ( !empty( $this->meta['image_meta']['shutter_speed'] ) ) {
			...
			$this->items['shutter_speed'] = array( sprintf( __( '%s sec', 'chromatic' ), $shutter_speed ), __( 'Shutter Speed', 'chromatic' ) );
		}
	}
	 ...
	 * function for that called `chromaticfw_get_audio_transcript()`.
	 *
	 * @since 1.0.0
	 * @access public
	 * @return void
	...
	public function audio_meta() {
		...
		if ( preg_match( '/^.*?\.(\w+)$/', get_attached_file( $this->args['post_id'] ), $matches ) )
			$this->items['file_type'] = array( esc_html( strtoupper( $matches[1] ) ), __( 'File Type', 'chromatic' ) );
	...
	public function video_meta() {
		...
		if ( !empty( $this->meta['width'] ) && !empty( $this->meta['height'] ) )
			/* Translators: Media dimensions - 1 is width and 2 is height. */
			$this->items['dimensions'] = array( sprintf( __( '%1$s &#215; %2$s', 'chromatic' ), number_format_i18n( absint( $this->meta['width'] ) ), number_format_i18n( absint( $this->meta['height'] ) ) ), __( 'Dimensions', 'chromatic' ) );
