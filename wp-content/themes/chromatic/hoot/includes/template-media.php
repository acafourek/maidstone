 ...
 * @return bool
 */
function hoot_attachment_is_audio( $post_id = 0 ) {
 ...
 * @return bool
 */
function hoot_attachment_is_video( $post_id = 0 ) {
 ...
 * @return int
 */
function hoot_get_attachment_id_from_url( $url ) {
	global $wpdb;
 ...
 * @return string
 */
function hoot_get_audio_transcript( $post_id = 0 ) {
	...
	return apply_filters( 'hoot_audio_transcript', $lyrics );
}
...
function hoot_attachment() {
	...
	foreach ( $mime_type as $type ) {
		if ( function_exists( "hoot_{$type}_attachment" ) )
			$attachment = call_user_func( "hoot_{$type}_attachment", $mime, $file );

		$attachment = apply_filters( "hoot_{$type}_attachment", $attachment );
	}

	echo apply_filters( 'hoot_attachment', $attachment );
}
 ...
 * @return string
 */
function hoot_application_attachment( $mime = '', $file = '' ) {
 ...
 * @return string
 */
function hoot_text_attachment( $mime = '', $file = '' ) {
	$embed_defaults = wp_embed_defaults();
 ...
 * @return string
 */
function hoot_audio_attachment() {
	return hoot_media_grabber( array( 'type' => 'audio' ) );
}
 ...
 * @return string
 */
function hoot_video_attachment() {
	return hoot_media_grabber( array( 'type' => 'video' ) );
}
