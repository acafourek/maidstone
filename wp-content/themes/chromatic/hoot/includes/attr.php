...
add_filter( 'hoot_attr_entry_summary',   'hoot_attr_entry_summary',   5    );
...
function hoot_attr_sidebar( $attr, $context ) {
	...
	if ( !empty( $context ) ) {
		...
		$attr['aria-label'] = esc_attr( sprintf( _x( '%s Sidebar', 'sidebar aria label', 'chromatic' ), hoot_get_sidebar_name( $context ) ) );
	}
...
function hoot_attr_menu( $attr, $context ) {
	...
	if ( !empty( $context ) ) {
		...
		$attr['aria-label'] = esc_attr( sprintf( _x( '%s Menu', 'nav menu aria label', 'chromatic' ), hoot_get_menu_location_name( $context ) ) );
	}
