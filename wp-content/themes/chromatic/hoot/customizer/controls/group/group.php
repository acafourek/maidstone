...
class Hoot_Customize_Group_Control extends WP_Customize_Control {
	...
	/**
	 * Enqueue scripts/styles for the group.
	 *
	 * @since 2.0.0
	 * @return void
	 */
	public function enqueue() {
		wp_enqueue_script( 'hoot-customizer-controls-group', trailingslashit( HOOTCUSTOMIZER_URI ) . "controls/group/script.js", array( 'jquery', 'wp-color-picker', 'customize-controls' ), HOOT_VERSION, true );
		wp_enqueue_style( 'hoot-customizer-controls-group', trailingslashit( HOOTCUSTOMIZER_URI ) . "controls/group/style.css", array(),  HOOT_VERSION );
	}
			...
			case 'group' :
				...
				switch ( $this->group ) {
					...
					case 'start' :
						...
						$button = empty( $this->button ) ? __( 'Edit Group', 'chromatic' ) : $this->button;
						?>
