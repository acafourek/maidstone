...
class Hoot_Customize_Betterbackground_Control extends WP_Customize_Control {
	...
	/**
	 * Enqueue scripts/styles for the betterbackground.
	 *
	 * @since 2.0.0
	 * @return void
	 */
	public function enqueue() {
		wp_enqueue_script( 'hoot-customizer-controls-betterbackground', trailingslashit( HOOTCUSTOMIZER_URI ) . "controls/betterbackground/script.js", array( 'jquery', 'wp-color-picker', 'customize-controls' ), HOOT_VERSION, true );
		wp_enqueue_style( 'hoot-customizer-controls-betterbackground', trailingslashit( HOOTCUSTOMIZER_URI ) . "controls/betterbackground/style.css", array(),  HOOT_VERSION );
	}
					...
					case 'button' :
						if (
							...
							<div class="hoot-betterbackground-buttons">
								<span class="button hoot-betterbackground-button hoot-betterbackground-button-predefined <?php if ( 'predefined' == $this->value() ) echo 'selected'; else echo 'deactive'; ?>" data-value="predefined"><?php echo __( 'Pattern', 'chromatic' ); ?></span><span class="button hoot-betterbackground-button hoot-betterbackground-button-custom <?php if ( 'custom' == $this->value() ) echo 'selected'; else echo 'deactive'; ?>" data-value="custom"><?php echo __( 'Custom Image', 'chromatic' ); ?></span>
							</div>
			...
			if ( $color || $image || $pattern ):
				...
				if ( $pattern ) :
					...
					$value[ "group-{$count}-p" ] = array(
						...
						'type'        => 'group',
						'priority'    => $setting['priority'],
						'button'      => '<span class="hoot-betterbackground-button-pattern"></span>' . __( 'Select Pattern', 'chromatic' ),
						'group'       => 'start',
					);
