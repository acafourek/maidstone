...
class ChromaticFw_Options_Media_Uploader {
	...
	static function chromaticoptions_uploader( $_id, $_value, $_desc = '', $_name = '' ) {
		...
		$output .= '<input id="' . esc_attr( $id ) . '" class="upload' . $class . '" type="text" name="' . esc_attr( $name ) . '" value="' . $value . '" placeholder="' . __('No file chosen', 'chromatic') .'" />' . "\n";
		if ( function_exists( 'wp_enqueue_media' ) ) {
			if ( ( $value == '' ) ) {
				$output .= '<input id="upload-' . esc_attr( $id ) . '" class="upload-button button" type="button" value="' . __( 'Upload', 'chromatic' ) . '" />' . "\n";
			} else {
				$output .= '<input id="remove-' . esc_attr( $id ) . '" class="remove-file button" type="button" value="' . __( 'Remove', 'chromatic' ) . '" />' . "\n";
			}
		} else {
			$output .= '<p><i>' . __( 'Upgrade your version of WordPress for full media support.', 'chromatic' ) . '</i></p>';
		}
	...
	static function enqueue_admin_options_media_scripts() {
		...
		wp_localize_script( 'chromaticfw-options-media-uploader', 'chromaticoptions_l10n', array(
			'upload' => __( 'Upload', 'chromatic' ),
			'remove' => __( 'Remove', 'chromatic' )
		) );
	}
