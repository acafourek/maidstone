<?php
/**
 * Helper Functions available in both admin and frontend for ChromaticFw Options Framework.
 * Remember that ChromaticFw Options Framework is loaded only in wpadmin (not loaded in frontend) and only for users
 * with 'edit_theme_options' capability. So functions here should be independent of ChromaticFw Options Framework.
 *
 ...
 * @return mixed
 */
function chromaticfw_get_option( $name, $default = NULL ) {
	...
	if ( $options === NULL ) {
		$option_name = chromaticoptions_option_name();
		$options = get_option( $option_name );
	}

	// Ideally, this should not have any expensive function, as chromaticfw_get_option() is used quite often.
	$options = apply_filters( 'chromaticfw_get_option', $options );
	...
	if ( isset( $options[$name] ) ) {
		...
		$override = apply_filters( 'chromaticfw_get_option_empty_values', false, $options[$name] );
		if ( $override !== true )
			return $options[$name];
	}
	...
	if ( $defaults === NULL ) {
		$options_array = chromaticfw_get_theme_options_array();

		// $defaults is created similar to get_default_values() in class-chromaticfw-options-admin
		if ( $options_array ) {
			foreach ( $options_array as $op ) {
				if ( isset( $op['id'] ) && isset( $op['std'] ) && isset( $op['type'] ) ) {
					if ( has_filter( 'chromaticfw_of_sanitize_' . $op['type'] ) ) {
						$defaults[ $op['id'] ] = apply_filters( 'chromaticfw_of_sanitize_' . $op['type'], $op['std'], $op );
					} else {
 ...
 * Allows for manipulating or setting options via 'chromaticfw_theme_options' filter
 * 
 * @since 1.0.0
 */
function chromaticfw_get_theme_options_array() {
	// cache, and to make sure only to include options.php once
	static $options = null;

	if ( !$options ) {
		// Load options from options.php file (if it exists)
		$base = str_replace( trailingslashit( THEME_DIR ), '', trailingslashit( CHROMATICFW_THEMEDIR ) );
		$location = apply_filters( 'chromaticfw_options_location', $base . 'admin/options.php' );
		if ( $optionsfile = locate_template( array( $location ) ) ) {
			$maybe_options = require_once $optionsfile;
			if ( is_array( $maybe_options ) ) {
				$options = $maybe_options;
			} else if ( function_exists( 'chromaticoptions_options' ) ) {
				$options = chromaticoptions_options();
			}
		}

		// Allow setting/manipulating options via filters
		$options = apply_filters( 'chromaticfw_theme_options', $options );
	}
 ...
 * @return void
 */
function chromaticfw_map_sortlist( $value, $sanitize_array = array(), $return = '' ) {
	$list = array(
		'order' => array(),
		'display' => array(),
		);
 ...
 * @since 1.0.0
 */
function chromaticfw_sort_slarray( $a, $b ) {
	$a = explode( ",", $a );
	$b = explode( ",", $b );
	$c1 = intval( $a[0] );
	$c2 = intval( $b[0] );
 ...
 * @since 1.0.0
 */
class ChromaticFw_Options_Helper {
	...
	static function get_pages( $number, $post_type = 'page' ){
		...
		// $options_pages[''] = __( 'Select a page:', 'chromatic' );
		$options_pages = array();
		$number = intval( $number );
	...
	static function get_posts( $number ){
		$options_posts[0] = __( 'Select a post:', 'chromatic' );
		$number = intval( $number );
	...
	static function typography_defaults() {
		...
		return apply_filters( 'chromaticfw_of_default_typography', array(
			'size' => '14px',
			'face' => $font,
			'style' => 'normal',
			'color' => '#333333' ) );
	...
	static function fonts( $return = 'websafe' ) {
		if ( $return == 'websafe' ) {
			if ( ! function_exists('chromaticfw_fonts_list') ) {
				if ( file_exists( trailingslashit( CHROMATICFWOPTIONS_DIR ) . 'includes/fonts.php' ) )
					require_once( trailingslashit( CHROMATICFWOPTIONS_DIR ) . 'includes/fonts.php' );
				else
					return array();
			}
			return chromaticfw_fonts_list();
		}

		if ( $return == 'google-fonts' || $return == 'google-font' ) {
			if ( ! function_exists('chromaticfw_googlefonts_list') ) {
				if ( file_exists( trailingslashit( CHROMATICFWOPTIONS_DIR ) . 'includes/fonts-google.php' ) )
					require_once( trailingslashit( CHROMATICFWOPTIONS_DIR ) . 'includes/fonts-google.php' );
				else
					return array();
			}
			return chromaticfw_googlefonts_list();
		}
	}
	...
	static function icons( $return = 'list' ) {
		if ( ! function_exists('chromaticfw_fonticons_list') )
			include_once( trailingslashit( CHROMATICFWOPTIONS_DIR ) . 'includes/icons.php' );

		if ( $return == 'sections' || $return == 'section' )
			return chromaticfw_fonticons_list('sections');

		$iconsArray = chromaticfw_fonticons_list('icons');
